...
public class ExtraPlaceholders {
    ...
    public static void init() {
        Placeholders.register(ResourceLocation.fromNamespaceAndPath("server", "objective_name_bottom"), (ctx, arg) -> {
            var args = arg.split(" ");
            if (args.length >= 2) {
        ...
        Placeholders.register(ResourceLocation.fromNamespaceAndPath("server", "objective_score_bottom"), (ctx, arg) -> {
            var args = arg.split(" ");
            if (args.length >= 2) {
                ServerScoreboard scoreboard = ctx.server().getScoreboard();
                Objective scoreboardObjective = scoreboard.getObjective(args[0]);
